...
from opencode_pair.workflow import intervene_task


class InterveneTaskTests(unittest.TestCase):
    def test_intervene_updates_config_and_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config, state = intervene_task(
                paths,
                note="focus on blockers",
                focus_only_blocking=True,
                developer_model="model-a",
                max_rounds=5,
            )
            self.assertTrue(config.focus_only_blocking)
            self.assertEqual(config.developer_model, "model-a")
            self.assertEqual(state.max_rounds, 5)
            self.assertEqual(state.intervention_note, "focus on blockers")
            self.assertEqual(state.intervention_count, 1)
